<template>
  <div>
    <v-toolbar class="grey darken-3" dark app fixed flat>
      <v-btn flat to="/myapprove/"><v-icon left>keyboard_arrow_left</v-icon> My Approve</v-btn>
    </v-toolbar>
    <v-layout column>
      <v-container grid-list-md>
        <v-layout row wrap>
          ...
          <v-flex xs12 sm12 md12 lg12>
            <cs-stepper :id="$route.params.id" :reload="reloadStepper" />
            <v-card>
              <v-toolbar color="primary" flat class="mb-2" dense>
                <v-spacer></v-spacer>
                <div v-if="isApprove">
                  <v-btn small dark @click="approved()"><v-icon small color="green" left>verified_user</v-icon>Approve</v-btn>
                  <v-btn small dark @click="rejected()"><v-icon small color="yellow darken-1" left>error_outline</v-icon>Reject</v-btn>
                </div>
              </v-toolbar>
              ...
              <v-card-text>
                <v-layout row>
                  <v-flex sm6>
                    <v-layout row>
                      <v-flex xs4 sm4 md4 lg4>
                          <v-subheader>FORMULIR NO</v-subheader>
                      </v-flex>                           
                      <v-flex xs8 sm8>                                                    
                    ...
                    <v-layout row>
                      <v-flex xs4 sm4 md4 lg4>
                          <v-subheader>DIBATALKAN OLEH</v-subheader>
                      </v-flex>                           
                      <v-flex xs8 sm8>                                                    
                  ...
                  <v-flex sm6>
                    <v-layout row>
                      <v-flex xs4 sm4 md4 lg4>
                          <v-subheader>DIMINTA OLEH</v-subheader>
                      </v-flex>                           
                      <v-flex xs7 sm7>                                                    
          ...
          <v-flex sm12 md12 lg12>
            <v-card>
              <v-toolbar color="white" flat>
                <v-toolbar-title>PERMINTAAN BARANG/REQUEST ITEMS</v-toolbar-title>
                <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    loadLists(){
      ...
      this.$http
        .post(this.$aid.url() + "ATKRequest/loadApprove_ByIDXRequest", id, {
          before: () => {
            this.spinner = true;
        ...
        .then(function(response) {
          ...
          console.log(arr);
          this.IDX_T_OutRequestForm = arr["Table"][0].IDX_T_OutRequestForm;
          ...
          this.isApprove = arr["Table"][0].isApprove;
          this.isReject = arr["Table"][0].isReject;
        })
        .then(function() {
          this.spinner = false;
        })
    ...
    approved(){
      ...
      this.ajaxCRUD(this.$aid.url() + "ATKRequest/approveApproveDetail", id, "Disabling...");
    },
    rejected(){
      var id = JSON.stringify({
        IDX_T_OutRequestForm: this.IDX_T_OutRequestForm,
      ...
      this.ajaxCRUD(this.$aid.url() + "ATKRequest/rejectApproveDetail", id, "Disabling...");
    },
    /**
     * 
     * @param {*} number 
